#include <iostream>
#include <string>
#include <regex> // Include for regular expressions
...
string sixDigits(int num)
{
    ...
    cout << int(num / 1000) << "\t" << thousand << endl;
    thousand = (thousand == "") ? "" : thousand + " Thousand";

    string nonthousand = threeDigits(int(num % 1000));
    nonthousand = (nonthousand == " Zero") ? "" : nonthousand;

    return "" +
           thousand + nonthousand;
}
...
string tenDigits(int num)
{
    ...
    // nonbillions = (nonbillions == "") ? "" : nonbillions + " Billion";
...
{
    regex pattern("^\\s+|\\s+$"); // Match leading and trailing whitespace
...
int main()
{
    string word = numberToWords(1000000000);
